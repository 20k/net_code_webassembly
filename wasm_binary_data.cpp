...
runtime::funcaddr runtime::store::allochostfunction(const types::functype& type, const std::function<std::optional<runtime::value>(const types::vec<runtime::value>&, runtime::store* s)>& ptr)
{
    int a = funcs.size();
...
runtime::memaddr runtime::store::allocmem(const types::memtype& type)
{
    ...
    inst.max = type.lim.get_max();
    inst.dat.resize(runtime::page_size * (uint32_t)type.lim.n+1);
...
void wasm_binary_data::self_test()
{
    ...
    types::vec<runtime::value> vals = s.invoke_by_name("_start", *m_minst,{});
